...
struct Edge {
	...
	//more specification: which side of src and dest?
	struct direction { // (from src), like connecting blocks
		char up;    // if .. then down
		char down;  // if .. then up
		char left;  // if .. then right
		char right;  // if .. left
	}
};
...
struct Graph* createGraph(struct Edge edges[], int n)
{
	...
	for (i = 0; i < N; i++) // 6 nodes to start with
		graph->head[i] = NULL;
	...
	for (i = 0; i < n; i++)
	{
		...
		newNode->dest = dest;  // where newNode will be inserted relative to src
